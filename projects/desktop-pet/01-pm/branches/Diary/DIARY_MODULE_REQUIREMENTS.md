...
| 边界类型 | 处理策略 |
| --- | --- |
| 截图采集失败（OS 拒绝、磁盘满、HDCP 阻断、应用未在前台等） | 静默降级到默认素材库，不打扰用户、不暴露失败原因。 |
| 用户撤回 `privacy_grants.diary_screenshot` | 立即清空本地全部截图缓存；所有依赖该档的历史日记自动降级到默认素材库重渲染，**不损坏日记正文与回信**。 |
| 日记软删除 | 同步清理该日记关联的本地截图。 |
| 关联 `highlight_event` 失效（user_deleted / consent_revoked / expired） | 对应截图同步失效，日记中该 slot 自动降级到下一档。 |
| 跨设备一致性 | 截图本地存储，不跨设备同步；用户在新设备打开历史日记时统一走 默认兜底。 |
