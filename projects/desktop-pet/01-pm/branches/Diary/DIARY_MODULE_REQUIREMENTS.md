...
| AI 生成图非主力 | P0 详情页插图以**用户高光时刻本地截图**为主，合作游戏方角色化素材库为次选；AI 生成图仅作为低证据兜底场景（独处小剧场、世界观推进等）的最后一档。截图采集需用户独立授权，原图仅本地保留，不上传记忆系统。装饰元素（贴纸、印章、胶带、角色立绘）仍由合作游戏方素材库提供。 |
...
| In-Scope | 高光时刻本地截图采集，并在对应日记中作为插图引用 | 仅在用户独立授权 `privacy_grants.diary_screenshot` 后触发，原图仅本地保留，不上传记忆系统。 |
...
| F-03-09 | 收信箱 | 视觉素材 | 详情页插图三档来源与降级链 | 全体桌宠玩家 | 详情页插图采用三档来源 + 自动降级链。① **首选 A：用户高光时刻本地截图**（来自客户端本地缓存，需独立授权，详见 F-04-09）；② **次选 B：合作游戏方角色化素材库**（按 episode / highlight 的 tags + category + scene 匹配检索）；③ **兜底 C：AI 生成临时图**（仅用于独处小剧场、世界观推进等低证据场景）。任一档不可用时自动降级至下一档，不出现无图日记的破损态。装饰元素（贴纸、印章、胶带、角色立绘）始终由合作游戏方素材库提供，与上述三档独立。 | P0 |

### 4. 日记详情阅读

| 功能ID | 一级模块 | 二级模块 | 功能点名称 | 使用角色 | 功能描述与业务规则 | 优先级 |
| --- | --- | --- | --- | --- | --- | --- |
| F-04-01 | 日记详情 | 图文排版 | 信纸 + 手账拼贴 | 全体桌宠玩家 | 1. 详情页左边为日记为底，包含标题、日期、正文段落、贴纸、来源摘要、回信区 | P0 |
| F-04-02 | 日记详情 | 来源摘要 | 轻量来源说明 | 全体桌宠玩家 | 1. 详情页标题区下方展示一条轻量来源说明（如"来自游戏事件和桌宠互动摘要"）。2. 严禁展示具体游戏数据原文、敏感证据、模型链路细节。3. 来源说明对用户不可关闭，作为透明度承诺。 | P0 |
...
| F-04-08 | 日记详情 | 图文关联 | 主图 / 辅图按降级链选择 | 全体桌宠玩家 | 1. 详情页可展示 0–2 张插图，由 content_angle 决定（高光时刻 1–2 张、普通成长 1 张、独处小剧场 0–1 张），**不强制最低张数**——没有值得呈现的瞬间就不放图。2. 主图（primary slot）来源优先级 A → B → C；辅图（secondary slot）通常走 B / C。3. 当 A 不可用（无截图 / 授权未开 / 缓存清理 / 采集失败）时静默降级到 B 或 C，不向用户暴露降级原因。4. 同一篇日记的图片在用户多次进入时保持一致，不随机切换。 | P0 |
| F-04-09 | 日记详情 | 截图采集授权 | 高光时刻本地截图采集 | 全体桌宠玩家 | 1. 截图采集需用户独立授权 `privacy_grants.diary_screenshot`，与桌面观察、聊天授权完全分离。2. **首次授权时机**：用户在桌宠陪伴期间首次发生高光事件时弹窗征求一次性同意；未同意则永久走 B/C 兜底，**不重复打扰**。3. **触发原则**：仅在 `highlight_event` 或 `idip_milestone` 命中时**尝试**截图；且事件本身需具备"值得纪念"特征（建议策略：source=user_saved 必触发；category 为 victory / milestone / funny 触发；emo 与平淡 episode 不触发）。**不强制每个 highlight_event 必有截图**——没有值得纪念的瞬间就不截图。4. 截图**仅在客户端本地保留**，不上传任何服务器；记忆系统仅持有本地 ID 与 content hash，不见原图。5. **保留期**：按生理日窗口组织，与对应日记生命周期绑定——日记被软删除时同步清理该日记关联的截图；用户撤回授权时立即清空全部本地截图，所有依赖截图的历史日记自动降级到 B/C 重渲染，**不损坏日记内容**。6. 截图采集失败（OS 拒绝 / 磁盘满 / HDCP 阻断 / 应用未在前台等）时静默走 B/C 兜底，不打扰用户。 | P0 |
  ...
  "illustrations": [
    {
      ...
      "local_ref": "local_id=shot_001;content_hash=sha256:abcd...",
      "asset_id": null,
      "generation_prompt_id": null,
      "fallback_chain": ["user_screenshot", "partner_asset", "ai_generated"],
      "fallback_used": "user_screenshot"
    },
    {
      "slot": "secondary",
      "source_type": "partner_asset",
      "local_ref": null,
      "asset_id": "asset_victory_001",
      "generation_prompt_id": null,
      "fallback_chain": ["partner_asset", "ai_generated"],
      "fallback_used": "partner_asset"
    }
  ]
}
```

> 注：原图二进制不在该接口返回。`source_type=user_screenshot` 时，前端通过 `local_ref` 直接读取客户端本地缓存；`local_ref` 仅含 local_id 与 content hash，不含原图。`fallback_chain` 为该 slot 配置的降级链，`fallback_used` 为实际命中的来源档。
...
| 边界类型 | 处理策略 |
|---|---|
| 截图采集失败（OS 拒绝、磁盘满、HDCP 阻断、应用未在前台等） | 静默降级到 B（素材库）或 C（生成图），不打扰用户、不暴露失败原因。 |
| 用户撤回 `privacy_grants.diary_screenshot` | 立即清空本地全部截图缓存；所有依赖该档的历史日记自动降级到 B/C 重渲染，**不损坏日记正文与回信**。 |
| 日记软删除 | 同步清理该日记关联的本地截图。 |
| 关联 `highlight_event` 失效（user_deleted / consent_revoked / expired） | 对应截图同步失效，日记中该 slot 自动降级到下一档。 |
| 跨设备一致性 | 截图本地存储，不跨设备同步；用户在新设备打开历史日记时统一走 B/C 兜底。 |
| 截图与正文一致性 | 若截图所引证据被撤销授权或删除，正文中相关引用片段也需经质检复核；正文未被影响时不重写。 |
...
| `illustration_refs[]` | array&lt;object&gt; | 插图引用列表（0–2 项），含 slot、source_type、对应来源标识与降级标记；详见 §五.4 illustrations[] 结构 |
