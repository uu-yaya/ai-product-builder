...
1. **§3.6 VLM 重写为三档混合架构**（接受 Radar Q-F1）：CNN 初筛 + 2-4B VLM 兜底 + 云端最终兜底（用户显式同意）；本地推理可用率目标从 70% 升 **≥85%**。仅押注 7B VLM 的可用率只有 35-45%，不达标。
...
4. **MVP 阶段 AI 范围**（v3 扩展）：atomic_facts + episode + profile + emotion_signal + current_context + **VLM 混合架构**（CNN + 2-4B VLM + 可选云端）+ MCP 摘要 + audio A0 派生（5s warmup）+ Playwright snippet；**不进 MVP**：Agent / Fine-tuning / 推荐系统 / 跨实例迁移 / 用户行为预测 / 音频内容识别 / 麦克风 / Playwright 后台轮询 / **仅 7B VLM 单押**。
...
| 2.11 | VLM 单 app 实例（游戏 + 视频，v3 重写为三档混合） | **必要** | **CNN 初筛 + 2-4B VLM 兜底 + 可选云端最终兜底** | 中（~2s VLM；~50ms CNN） | **高**（VLM）/ **低**（CNN） | **极高** | ✅ P1 | 三档逐级降级；最终回退 `unknown`，不强行猜 |
...
[ 档 1: CNN 初筛 ]  ← MobileNet / EfficientNet 量级，~50ms / 帧，~10MB 模型
        ...
        ├─→ 低置信 / 罕见场景 ──┐
        │                       ▼
        │              [ 档 2: 2-4B VLM 兜底（本地）] ← MiniCPM-V-2 / Qwen2.5-VL-3B 量级，~2GB 显存
        │                       │
        │                       ├─→ 输出 tag + 短摘要 → 写入记忆
        │                       │
        │                       └─→ 仍低置信 / 模型不可用 ──┐
...
2. **2-4B VLM 是中档**，覆盖 CNN 不能搞定的中等难度场景；本地推理，~2GB 显存可跑（核显 / 低配 GPU 均可）。
3. **云端 VLM 是受限最终兜底**，仅在用户 onboarding 显式同意（"我同意低难度场景脱敏后上传云端 VLM 增强"）后才启用；上传**仅脱敏帧**（缩略 + 高斯模糊关键区域），不上传原图。
4. **任何档失败回退 `unknown`**，桌宠不假装看到。

#### 3.6.1.2 与 v2 "VLM 本地优先" 的关系

1. v2 §3.6.4 "本地优先" **仍然成立**：CNN（本地）+ 2-4B VLM（本地）是主路径，覆盖 ~85%。
...
2. 候选模型：本地 VLM 7B 级（2026 年消费级 GPU 已可跑）；如硬件不支持，**仅返回 `unknown`**，不强制走云端。
3. 云端兜底（如必要）：
   1. **仅上传脱敏帧**（缩略图 + 高斯模糊处理后），不上传原图。
   2. **仅返回语义级标签**，不返回完整帧分析。
   3. 每次激活必须用户显式确认（不是后台静默）。
4. 输出严格限制：
   1. `semantic_tags` ≤ 5 个；
...
1. **本地可用率 ≥85%**（v3 修订）：在目标用户硬件分布下，三档混合（CNN 初筛 + 2-4B VLM 兜底 + 云端兜底用户同意率）合计可用率 ≥85%。仅 7B VLM 路径 35-45% 已被 Radar 验证不达标。
2. **CNN 初筛覆盖率 ≥60%**（v3 新增）：CNN 在常见场景（boss_fight / scene_funny / scene_action / idle / video_loading 等 ~15 类）上的命中率；命中即不调 VLM。
3. **VLM 兜底准确率 ≥75%**（v3 沿用 v2 数值）：抽样 200 个 CNN 未命中场景人工标注 vs 2-4B VLM 输出准确率。
...
6. **VLM 三档混合架构**（单 app 实例 = 游戏 + 视频）（v3 重写）：
   ...
   2. 2-4B VLM 兜底（本地，MiniCPM-V 4.5 / Qwen2.5-VL-7B int4，覆盖中等难度场景）
...
| 5 | VLM 在 2026 年本地推理可用率 70% 假设是否成立？需要 Engineering / Radar 真实硬件分布数据。 | Engineering + Radar | 影响 §3.6 是否需要扩展云端方案 |
