...
| 系统 | 负责 | 不负责 |
| --- | --- | --- |
| 客户端 | 1. 接收 Game SDK 数据； 
2. 采集 PC 低敏状态； 
3. 本地 VLM 处理； 
4. 生成本地 `current_context`； 
5. 向记忆系统上报事实源； 
6. 消费记忆系统上报的数据； 
7. 回写用户反馈 / 保存 / 删除 / 纠错后的数据； | 1. 不长期保存完整记忆； 
2. 不把临时判断伪装成事实； 
3. 不把高敏原始数据上传 |
| 记忆系统 | 1. 接收标准化事实源；存储；加工；生成 atomic facts / episode / profile / highlight / digest；支持 query；在加工结果变化时 push 轻摘要；处理删除 / 纠错 / 授权状态 | 不直接采集游戏 SDK；不直接读用户屏幕；不接收 VLM 原图作为长期数据；不决定桌宠当下是否开口 |
