...
| 系统 | 负责 | 不负责 |
|---|---|---|
| 客户端 Client | 接收 Game SDK 数据；采集 PC 低敏状态；本地 VLM 处理；生成本地 `current_context`；向记忆系统上报事实源；消费记忆系统返回；回写用户反馈 / 保存 / 删除 / 纠错 | 不长期保存完整记忆；不把临时判断伪装成事实；不把高敏原始数据上传 |
| 记忆系统 Memory System | 接收标准化事实源；存储；加工；生成 atomic facts / episode / profile / highlight / digest；支持 query；在加工结果变化时 push 轻摘要；处理删除 / 纠错 / 授权状态 | 不直接采集游戏 SDK；不直接读用户屏幕；不接收 VLM 原图作为长期数据；不决定桌宠当下是否开口 |
| 游戏 SDK Game SDK | 向客户端提供游戏生命周期、状态快照、实时事件和游戏自定义字段 | 不直接绕过客户端写入记忆系统 |
