...
数据对象分类只描述 Envelope 内 payload 的业务性质，不等于传输格式。Client ↔ Memory 之间的实际数据传送最终都必须按第 3 章用 Envelope 包装：`source_record` 通过上报 Envelope 发送，`user_control_state` 通过 query / mutation / consent Envelope 同步，`derived_memory` 通过 query response / push Envelope 返回。

`current_context` 不列入本表。它是客户端本地运行时状态，不是跨系统传输的 payload；只在 4.8 单独说明。
...
| `current_context` 不完整回写 | `current_context` 是客户端本地运行时状态，不是长期事实，也不是对话内容；必要时只回写事实源或业务确认后的子结果，并按第 3 章 Envelope 发送 |
