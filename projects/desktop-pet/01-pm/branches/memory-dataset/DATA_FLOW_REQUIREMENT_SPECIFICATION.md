...
| 系统 | 负责 | 不负责 |
| --- | --- | --- |
| 客户端 | 1. 接收 Game SDK 数据； | 1. 不长期保存完整记忆； |
...
| 记忆系统 | 1. 接收标准化事实源； | 1. 不直接采集游戏 SDK； |
|  | 2. 存储、加工数据，生成 atomic facts / episode / profile / highlight / digest等 | 2. 不直接读用户屏幕； |
|  | 3. 支持 query,在加工结果变化时 push 轻摘要 | 3. 不接收 VLM 原图作为长期数据 |
|  | 4. 处理删除 / 纠错 / 授权状态 | 4. 不决定桌宠当下是否开口 |
