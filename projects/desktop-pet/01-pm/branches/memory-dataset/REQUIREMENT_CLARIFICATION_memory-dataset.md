...
## 0.9 数据来源分类架构原则（v2.6 新增，2026-05-12 — 消费方接入驱动）
...
记忆系统提供包括 memory **采集层** + memory **派生层**（含 AI 派生，但**结果 stored**）。
消费侧生成受**桌宠 IP / 当下场景 / 用户偏好**影响，每次按需生成，**stored 会失效**。
...
| 2 | **memory schema 字段必须 >1 消费方复用** | 单消费方一次性用的内容进 消费侧生成（消费侧生成），避免 memory schema 膨胀 |
| 3 | **AI 派生层级（A vs B）以 AI Eval §3 各候选点为准** | atomic_facts / episode / profile / emotion / persona 进 A；对话回复 / 日记正文 / 卡片文案进 B |
| 4 | **消费侧生成生成不写回 memory** | 例外：profile.summary 用户点"重新总结"由消费侧 LLM 生成后写回 memory（记忆系统常驻 + 消费侧触发更新混合） |
...
| 问题 | 答 "是" → | 答 "否" → |
|---|---|---|
| Q1：≥2 个消费方使用同一份结果？ | 记忆系统候选 | 消费侧候选（一次性使用） |
| Q2：结果不受桌宠 IP 影响？ | 记忆系统候选 | 消费侧（受 IP，每次按 IP 生成） |
| Q3：结果不受当下场景 / 用户偏好影响？ | 记忆系统候选 | 消费侧（动态） |
| Q4：结果在 AI Eval §3 已有派生候选点？ | 记忆系统（沿用 AI Eval 立场） | 看 Q1-Q3 综合判断 |

四问全 "是" → 记忆系统提供；任一 "否" → 消费侧生成；不确定 → 默认消费侧生成（避免 memory schema 误膨胀）。
...
2. **新增 §0.9 来源 数据来源分类架构原则**：记忆系统提供 = memory 直接产出（schema 定义字段）；消费侧生成 = memory 提供前置数据，消费方 LLM / 代码二次转换（不进 schema）；附三层数据架构图 + PM 红线 5 条 + 性质模糊时 4 问归类。
3. **日记 3 字段补**（记忆系统提供，进现有数据类）：
...
5. **新增 §4.18 消费侧生成消费侧动态生成内容清单**（不进 memory schema）：8 项 — 日记正文 / 一句话画像卡片 / pet_observation / 桌宠对话 / 分享图文 / persona 解释 / 里程碑文案 / profile.summary 重写（B 触发更新例外）。
6. **§4.1 / §4.6 字段表加 来源 标注列**：现有字段不动，加注 来源 归属与派生方式。
7. **PM 红线汇总**（v2.6）：
   1. memory 不存"桌宠语气"内容（pet_observation / 对话 / 日记正文 / 卡片文案受 IP 影响走 消费侧生成）。
   2. memory schema 字段必须 >1 消费方复用（单消费方一次性进 B）。
   3. AI 派生层级（A vs B）以 AI Eval §3 各候选点为准。
   4. 消费侧生成生成不写回 memory（例外：profile.summary 重写）。
...
| 编号 | 字段 | 来源 | 来自记忆系统的层 | 桌宠侧用途 |
|---|---|---|---|---|
| 1 | `atomic_facts[]` | 记忆系统 | 原子事实层（AI 派生） | 短期对话引用："你刚说过 …" |
| 2 | `episode.title` + `episode.time_range` | 记忆系统 | 情节摘要层（AI 派生） | 跨日 / 跨 session 召回："昨天聊到的那个秒杀模块" |
| 3 | `profile.summary` | A（常驻）+ B（重写触发） | 用户画像层（AI 派生） | 风格定调；用户点"重新总结一下我"由消费侧 LLM 生成后写回 memory |
| 4 | `profile.interests[]` + `profile.preferences[]` | 记忆系统 | 用户画像层 | 主动话题选取 |
| 5 | `profile.behavior_patterns[]` | 记忆系统 | 用户画像层 | 时机判断（如"晨会摸鱼时间"是已知行为模式） |
| 6 | `profile.key_facts[]` | A（LLM 候选 + 用户确认） | 用户画像层 | 重要事实优先注入对话上下文；未确认 → 不进对话引用 |
| 7 | `profile_meta`（新增建议） | 记忆系统 | 元字段层 | 每条 profile 的置信度 / 来源 / 时间，避免假记忆 |
| 8 | `emotion_signal`（**v2 锁定进 MVP**） | 记忆系统 | 推导层（AI Eval §3.4） | 情绪倾向：紧张 / 兴奋 / 沮丧 / 平静 |
| 9 | **`atomic_facts.quote_eligible`**（v2.6 新增 — 日记原话引用） | 记忆系统 | 派生层（规则代码 / PII 检测） | 日记会引用用户原话，部分原话含 PII（真名 / 同事抱怨 / 财务等）；本字段标记"哪些原话可引用"；正则 + NER 在生产 atomic_facts 时同步标记 |
...
| 编号 | 字段 | 来源 | 形态 | 桌宠侧用途 |
|---|---|---|---|---|
| 1 | `game_session.start` / `game_session.end` | 记忆系统 | 事件 | 知道一局开始结束 |
| 2 | `game_session.in_game_time` | 记忆系统 | 实时推送 | 游戏内时间（早 / 晚、第几日等，部分游戏） |
| 3 | `game_event.stream`（open / death / revive / settlement 等） | 记忆系统 | 事件流 | 桌宠按事件做反应 |
| 4 | `game_session.duration_signal`（>30min / >1h / >3h 分级） | 记忆系统 | 衍生信号 | 健康提示触发点 |
| 5 | **`event_emotion_tag`**（v2.6 新增 — 事件级情绪标签） | 记忆系统 | 派生层（AI，复用 AI Eval §3.4 emotion_signal 扩到事件级） | 取值：joy / frustrate / satisfy / sadness / neutral；日记 / 高光 / 复盘需要"某一局结束时"的情绪锚点（与 §4.8 current_context.mood_estimate 滑窗级互补） |
| 6 | **`episode.highlight_score`**（v2.6 新增 — 派生到 §4.1 episode） | 记忆系统 | 派生层（规则代码） | 0-1 浮点；从 idip_milestone × emotion_strength × user_action 加权派生；日记筛选阈值 + 高光候选输入；多消费方复用 |
...
#### 4.13.2 期望字段（全部 记忆系统提供）

| 编号 | 字段 | 来源 | 派生方式 | 桌宠 / Memory Center 用途 |
|---|---|---|---|---|
| 1 | `highlight_id` | 记忆系统 | 系统生成 | 主键，多消费方引用 |
| 2 | `title` | 记忆系统 | LLM 候选 + ✅ 用户可编辑 | 标题（短，≤20 字） |
| 3 | `time` | 记忆系统 | 系统直接 | 事件时间戳 |
| 4 | `scene` | 记忆系统 | LLM 派生（合成 `active_app` + `window_title_redacted` + `game_vlm.semantic_tags`） | 场景描述（如"王者荣耀 BOSS 战翻盘"） |
| 5 | `event_summary` | 记忆系统 | LLM 生成 + ✅ 用户可编辑 | 事件摘要（≤80 字；引用 `atomic_facts` 时必须 `quote_eligible=true`） |
| 6 | `category` | 记忆系统 | LLM 推荐 + ✅ 用户可改 | 枚举：`achievement` / `growth` / `emotion` / `social` / `collection` / `relationship` |
| 7 | `tags[]` | 记忆系统 | LLM 候选 + ✅ 用户可改 | 自由标签（≤5 条，去重） |
| 8 | `source` | 记忆系统 | 系统直接 | 枚举：`idip_milestone` / `episode_highlight_score` / `user_starred` / `persona_result` |
| 9 | `privacy_level` | 记忆系统 | 用户默认（来自 `user_preferences.privacy_grants`）+ ✅ 可改 | 枚举：`private` / `shareable`；私有不允许进分享卡片生成 |
...
#### 4.16.2 期望字段（全部 记忆系统提供）

| 编号 | 字段 | 来源 | 派生方式 | 桌宠 / 画像页用途 |
|---|---|---|---|---|
| 1 | `assessment_id` | 记忆系统 | 系统生成 | 主键 |
| 2 | `persona_label` | 记忆系统 | 公司既有平台 → memory 存结果 | 该游戏人格标签（如"团战指挥型"） |
| 3 | `persona_schema_version` | 记忆系统 | 公司平台 | 分类体系版本（防 meta 漂移；版本升级时旧结果标过期） |
| 4 | `similarity_breakdown[]` | 记忆系统 | 公司平台 | `[{candidate, score (0-1), evidence_ids[]}]`；候选人格相似度 + 证据 ID 指向 game_event / idip_milestone（用于"为什么像 TA"页面展开） |
| 5 | `assessment_at` | 记忆系统 | 系统直接 | 测定时间 |
| 6 | `data_window` | 记忆系统 | 系统直接 | `{from, to, session_count}`；测定基于的数据窗口（"最近 30 天 60 局"） |
| 7 | `user_feedback` | 记忆系统 | ✅ 用户 | enum：`accepted` / `rejected` / `not_like_me`；反哺下次测定权重 |
| 8 | `evidence_strength` | 记忆系统 | 系统直接 | 支持证据条数（**≥3 红线**） |
| 9 | `is_expired` | 记忆系统 | 派生（基于 `persona_schema_version` 与当前版本对比） | bool；版本升级后旧结果标过期（UI 提示"旧版本测定，是否重测"） |
...
1. **记忆系统提供（对消费方）**：消费方 query memory 视图 API 直接拿；**实现是派生**（不持久化原子存储）。
2. **PM 边界**（沿用用户清单）：纯纪念性，**不做强压力任务**；不出现"未完成任务 / 必须达成 X 才能解锁"压力 UI。
3. 关系等级的计算公式由 Engineering 决（PM 不锁数字），但 PM 守住"加权指标都来自数据层 + 不掺非数据指标"原则。

#### 4.17.2 期望字段（派生视图，全部 记忆系统提供）

| 字段 | 类型 | 来源 | 派生源 |
|---|---|---|---|
| `companion_days` | int | 记忆系统 | first_install_at → now（按日历日） |
| `co_game_session_count` | int | 记忆系统 | count(game_event.session) |
| `diary_count` | int | 记忆系统 | count(episode where highlight_score ≥ diary_threshold) |
| `highlight_count` | int | 记忆系统 | count(highlight_event WHERE deleted_at IS NULL) |
| `persona_assessment_count` | int | 记忆系统 | count(game_persona_assessment) |
| `relationship_level` | int | 记忆系统 | 上述指标加权派生（公式 Engineering 决） |
| `milestone_unlocked[]` | string[] | 记忆系统 | 派生：陪伴 7 / 30 / 100 / 365 天；首条高光；首次测定；首条日记 等等 |
...
### 4.18 消费侧动态生成内容清单（v2.6 — 不进 memory schema）
