...
## 0.9 数据来源分类架构原则（v2.6 — 消费方接入驱动）

> 此节给所有读者的"数据来源链路"通用速查。本分支所有字段在实现 / 消费时都必须归到"记忆系统提供"或"消费侧派生 / 生成"之一；性质不清时按 §0.9.6 的 4 问归类。
...
```
┌──────────────────────────────────────────────────────────────┐
│ 记忆系统提供层（数据底座 — 采集 + 用户输入，进 schema）            │
│                                                              │
│ ① OS / 通道 / SDK 直采                                        │
│   active_app / window_title_redacted / is_fullscreen        │
│   idle 时间 / focused_element_role / ui_text_snapshot       │
│   chat_text 原 / game_session.* / game_event.stream 原      │
│   idip_snapshot 原 / idip_field_metadata 配置                │
│   MCP / OS API / 浏览器 / OSA / CLI / IFTTT 通道元数据原文    │
│   audio A1 Now Playing 元数据                                │
│   behavior A2 OS 操作语义事件（save / copy / lock 等直采）   │
│ ② 用户主动输入                                                │
│   user_preferences.*（含 privacy_grants）                    │
│   profile.gameplay_motivation[] 已确认 / key_facts[] 已确认  │
│   highlight_event 的 pinned / privacy_level 用户改后版本     │
│   profile.* 用户编辑后版本（user_corrected=true）            │
│ ③ 系统记录                                                    │
│   IDs / 时间戳 / source_category / first_seen_at            │
│   user_corrected 标记 / feedback_history[]                  │
└──────────────────────────────────────────────────────────────┘
                              ↓ 提供前置数据
┌──────────────────────────────────────────────────────────────┐
│ 消费侧派生 / 生成层（不在记忆系统责任范围）                       │
│                                                              │
│ ① 派生计算（规则 / 计算 / 信号处理）                              │
│   idip_delta / idip_anomaly / idip_milestone                │
│   behavior A1 派生指标（强度 / 节奏 / 桶化）                    │
│   behavior A3 编辑动作派生（IME / undo 频率 等）               │
│   audio A0（BPM / energy / beat — 本地信号处理）              │
│   relationship_stats.* / episode.highlight_score             │
│   game_session.duration_signal / idle_signal 分级           │
│   profile_meta.confidence / decay_score                     │
│ ② LLM 派生（AI 抽取 / 摘要 / 推断 / 分类）                       │
│   atomic_facts / episode / profile.summary / interests      │
│   profile.preferences / behavior_patterns / personality     │
│   profile.gameplay_motivation_candidates[]                  │
│   profile.gameplay_style_tags[]                             │
│   emotion_signal / event_emotion_tag                        │
│   current_context.*（activity_topic / mood / interrupt）    │
│   highlight_event 的 title / scene / event_summary /        │
│     category / tags                                          │
│   atomic_facts.quote_eligible（PII 检测）                     │
│ ③ 外部平台派生                                                │
│   VLM semantic_tags / user_visible_summary（模型推理）        │
│   game_persona_assessment.persona_label /                   │
│     similarity_breakdown（公司既有平台）                       │
│ ④ 实时动态生成（按桌宠 IP / 当下场景每次生成）                    │
│   日记正文 / pet_observation / 桌宠对话回复                  │
│   一句话画像卡片 / 分享图文 / persona 解释文案                 │
│   里程碑卡片文案 / 关系等级展示文案                            │
└──────────────────────────────────────────────────────────────┘
```

> 派生 / 生成的**结果**可以写回 memory schema 持久化（避免重算 + 多消费方复用），但**派生工作的责任归消费侧**，不是记忆系统职责。是否写回是 Engineering 实现优化，详见 §0.9.3。

### 0.9.3 边界：派生结果是否写回 memory（实现优化层）

| 场景 | 是否写回 memory | 例子 |
|---|---|---|
| 多消费方复用 + 重算成本高 + 内容不依赖 IP | ✅ 写回（性能 / 一致性） | atomic_facts / episode / profile / highlight_event 字段 / persona_assessment / current_context.mood_estimate |
| 实时 / 一次性 / 受 IP 场景影响 | ❌ 不写回（每次按需生成） | 日记正文 / pet_observation / 对话回复 / 卡片文案 / persona 解释文案 |
| 信号处理派生（高频 / 短期） | 看 Engineering 决（建议 short-term buffer） | A0 BPM / A1 派生指标 / idip_delta |

> **关键判断**：写回 memory 是**实现优化**，不改变"派生责任归消费侧"的归属判断。在字段表中"来源"列标"消费侧"的字段，即使结果在 memory schema 里有对应字段定义，**派生工作仍由消费侧团队负责**。

### 0.9.4 消费侧派生 / 生成内容总览（按 4 类，详见 §4.18）

| 类 | 触发 | 数据源（从 memory 拿什么前置） | 实现 |
|---|---|---|---|
| **① 派生计算** | 上游数据更新时 / 周期性 | 原始字段（idip_snapshot / game_event / behavior 信号 / chat / 音频） | 规则代码 / 计数 SQL / 信号处理库（如 aubio） |
| **② LLM 派生** | chat segment 闭合 / 周期性 / 用户触发 | chat / behavior / game_event / idip 原始 | AI Agent（atomic_facts 抽取 / episode 摘要 / profile 生成 / emotion / current_context / highlight LLM 字段 / PII 检测 / 等） |
| **③ 外部平台派生** | VLM 触发 / persona 测定触发 | active_app + 屏幕 buffer / game_event + idip_milestone | VLM 推理服务（CNN + 量化 7-8B VLM + 云端兜底）/ 公司既有人格测定平台 |
| **④ 实时动态生成** | UI 加载 / 用户交互 / 升级触发 | memory 全套 + 桌宠 IP | 桌宠 agent / 日记生成器 / UI Composer / 分享服务 |

### 0.9.5 PM 架构红线

| # | 红线 | 理由 |
|---|---|---|
| 1 | **记忆系统不做派生** | 派生 = 业务逻辑层；记忆系统是数据底座。写回 memory 是实现优化，不改归属 |
| 2 | **memory 不存"桌宠语气"内容** | pet_observation / 对话 / 日记正文 / 分享文案受 IP 影响；写回失效 |
| 3 | **AI 派生层级以 AI Eval §3 各候选点为准** | atomic_facts / episode / profile / emotion / persona 等的 AI 实施细节在 AI Eval；本文档只锁数据归属 |
| 4 | **persona 不作为陪伴策略的直接驱动信号 / 不反向污染采集层** | 沿用 §4.16 红线，防过拟合 |
| 5 | **`user_preferences.privacy_grants` 默认全 false，强制用户主动 + 不可推断** | PRIVACY_BOUNDARY 二级开关 |

### 0.9.6 性质模糊时的默认归类（4 问）

未来出现新字段 / 新数据需求时，按下表归类：

| 问题 | 答 "是" → | 答 "否" → |
|---|---|---|
| Q1：是 OS / 游戏 SDK / 通道 SDK 直接采集到的吗？ | 记忆系统候选 | 看 Q2 |
| Q2：是用户主动输入 / 主动确认的吗？ | 记忆系统候选 | 看 Q3 |
| Q3：需要 LLM / 规则 / 计算 / 外部平台才能产出吗？ | **消费侧派生** | — |
| Q4：受桌宠 IP / 当下场景影响吗？ | **消费侧生成**（且**一定不写回 memory**） | 看 Q1-Q3 |

四问综合：Q1 或 Q2 任一为是 → 记忆系统提供；Q3 / Q4 任一为是 → 消费侧。同时满足 Q1 和 Q3（少见）按"主导工作量"归类，多数归消费侧。

### 0.9.7 字段详细产出方式索引

本文档（PM 工作底稿）§4.x 字段表的"产出方式"列保持简略口径（产出大类 + 关键约束）。**各字段详细产出方式**（具体 OS API 名 / 模型选型 / 触发条件 / 实现工具链 / Engineering 落地细节）见 [`DATA_REQUIREMENT_SPECIFICATION_memory-dataset.md`](DATA_REQUIREMENT_SPECIFICATION_memory-dataset.md) 评审稿 §2.x 对应章节（章节号一一对应：本文 §4.X ↔ DRS §2.X）。

| 本文档 § | DRS § | 模块 |
|---|---|---|
| §4.1 | §2.1 | chat |
| §4.2 | §2.2 | behavior_pc |
| §4.3 | §2.3 | behavior 用户操作（含 A1/A2/A3 + L1.5） |
| §4.4 | §2.4 | APP 6 通道 |
| §4.5 | §2.5 | game_idip |
| §4.6 | §2.6 | game_event |
| §4.7 | §2.7 | VLM |
| §4.8 | §2.8 | current_context |
| §4.9 | §2.9 | profile_meta |
| §4.10 | §2.10 | 音频派生 |
| §4.11 | §2.11 | Playwright |
| §4.12 | §2.12 | OSA Bridge |
| §4.13 | §2.13 | highlight_event |
| §4.14 | §2.14 | user_preferences |
| §4.15 | §2.15 | profile 扩展 |
| §4.16 | §2.16 | game_persona_assessment |
| §4.17 | §2.17 | relationship_stats |
| §4.18 | §2.18 | 消费侧派生 / 生成清单 |
...
| 编号  | 字段                                                  | 来源               | 产出方式                                  | 桌宠侧用途                                                                               |
| --- | --------------------------------------------------- | ---------------- | ------------------------------------- | ----------------------------------------------------------------------------------- |
| 1   | `chat_text` 原文                                      | 记忆系统             | OS / chat runtime 直采                  | 给消费侧 LLM 提供前置数据 — 抽取 / 摘要 / 推断的原料                                                  |
| 2   | `atomic_facts[]`                                    | 消费侧派生            | LLM 抽取（结果可写回 memory 持久化）              | 短期对话引用："你刚说过 …"                                                                     |
| 3   | `episode.title` + `episode.content` + `episode.time_range` | 消费侧派生         | LLM 摘要（结果可写回 memory 持久化）              | 跨日 / 跨 session 召回："昨天聊到的那个秒杀模块"                                                     |
| 4   | `profile.summary`                                   | 消费侧派生（触发更新）       | LLM 生成（写回 memory 常驻；用户点"重新总结一下我"触发重写） | 风格定调                                                                                |
| 5   | `profile.interests[]` + `profile.preferences[]`     | 消费侧派生 + 用户编辑可改   | LLM 抽取 + 用户在 Memory Center 增删 / 纠错     | 主动话题选取                                                                              |
| 6   | `profile.behavior_patterns[]`                       | 消费侧派生            | LLM 抽取（用户只读 / 可删，不可编辑）                | 时机判断（如"晨会摸鱼时间"是已知行为模式）                                                              |
| 7   | `profile.key_facts[]`                               | 消费侧派生 → 用户确认才生效   | LLM 候选；未确认前**不进对话引用**；确认后存为记忆系统输入   | 重要事实优先注入对话上下文                                                                       |
| 8   | `profile.personality_traits[]`                      | 消费侧派生 + 用户"不像我"反馈 | LLM 推断；用户不可直接编辑，仅可标 "不像我"             | 个性描述                                                                                |
| 9   | `profile_meta.source_category` / `first_seen_at` / `last_confirmed_at` / `user_corrected` / `feedback_history[]` | 记忆系统       | 系统记录元数据                              | Memory Center 来源解释 + 时效衰减 + 纠错锁定                                                    |
| 10  | `profile_meta.confidence` / `decay_score`           | 消费侧派生            | 规则映射 + LLM 自评混合公式 / 时间衰减派生            | 引用前判断"是否说出来"                                                                        |
| 11  | `emotion_signal`                                    | 消费侧派生            | AI 推导（AI Eval §3.4）                   | 情绪倾向：紧张 / 兴奋 / 沮丧 / 平静                                                              |
| 12  | `atomic_facts.quote_eligible`（v2.6 新增 — 日记原话引用）   | 消费侧派生            | 规则 PII 检测（正则 + NER；生产 atomic_facts 时同步标记） | 日记 / 高光引用原话时只能用 quote_eligible=true 的子集                                              |
...
| 编号  | 字段                                                        | 来源   | 产出方式                                       | 桌宠侧用途                                                                                                                    |
| --- | --------------------------------------------------------- | ---- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 1   | `game_session.start` / `game_session.end`                 | 记忆系统 | 游戏 SDK 直采事件                                  | 知道一局开始结束                                                                                                                 |
| 2   | `game_session.in_game_time`                               | 记忆系统 | 游戏 SDK 实时推送                                  | 游戏内时间（早 / 晚、第几日等，部分游戏）                                                                                                   |
| 3   | `game_event.stream`（open / death / revive / settlement 等） | 记忆系统 | 游戏 SDK pub/sub                               | 桌宠按事件做反应                                                                                                                 |
| 4   | `game_session.duration_signal`（>30min / >1h / >3h 分级）     | 消费侧派生 | 规则代码（从 game_session.start 时间戳计算）            | 健康提示触发点                                                                                                                  |
| 5   | `event_emotion_tag`（v2.6 新增 — 事件级情绪标签）                  | 消费侧派生 | AI 推断（复用 AI Eval §3.4 emotion_signal 扩到事件级） | 取值：joy / frustrate / satisfy / sadness / neutral；日记 / 高光 / 复盘需要"某一局结束时"的情绪锚点（与 §4.8 current_context.mood_estimate 滑窗级互补） |
| 6   | `episode.highlight_score`（v2.6 新增 — 派生到 §4.1 episode）   | 消费侧派生 | 规则代码（idip_milestone × emotion_strength × user_action 加权） | 0-1 浮点；日记筛选阈值 + 高光候选输入；多消费方复用                                                                                          |
...
3. **记忆系统提供全字段**（含 LLM 派生字段），由 memory 派生层一次性合成 + stored；用户可在 Memory Center 编辑（user_corrected 锁定）。
4. **桌宠 IP 语气的"宠物观察视角文本"不在本数据类**（消费侧生成，消费侧每次按 IP 生成；见 §4.18 #3）。

#### 4.13.2 期望字段

> **记忆系统侧字段**：highlight_id / time / source / pinned / privacy_level / evidence_ids（系统记录 + 用户输入）；用户编辑后的 title / event_summary / category / tags（确认后存为记忆系统输入）。
> **消费侧派生字段**：title / scene / event_summary / category / tags 的初始候选值（LLM 派生后写回 memory 持久化；用户编辑后覆盖为记忆系统输入）。

| 编号  | 字段               | 来源              | 产出方式                                                                         | 桌宠 / Memory Center 用途                                                               |
| --- | ---------------- | --------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 1   | `highlight_id`   | 记忆系统            | 系统生成                                                                         | 主键，多消费方引用                                                                           |
| 2   | `title`          | 消费侧派生 → 用户可编辑覆盖 | LLM 候选；用户编辑后 user_corrected=true，LLM 不再覆盖                                  | 标题（短，≤20 字）                                                                         |
| 3   | `time`           | 记忆系统            | 系统直接（来自上游 event 时间戳）                                                          | 事件时间戳                                                                               |
| 4   | `scene`          | 消费侧派生            | LLM 合成（`active_app` + `window_title_redacted` + VLM semantic_tags）           | 场景描述（如"王者荣耀 BOSS 战翻盘"）                                                              |
| 5   | `event_summary`  | 消费侧派生 → 用户可编辑覆盖 | LLM 生成（≤80 字；引用 `atomic_facts` 时必须 `quote_eligible=true`）；用户编辑后覆盖             | 事件摘要                                                                                |
| 6   | `category`       | 消费侧派生 → 用户可改   | LLM 分类推荐；用户改后覆盖                                                              | 枚举：`achievement` / `growth` / `emotion` / `social` / `collection` / `relationship`  |
| 7   | `tags[]`         | 消费侧派生 → 用户可改   | LLM 候选（≤5 条）；用户改后覆盖                                                          | 自由标签                                                                                |
| 8   | `source`         | 记忆系统            | 系统直接（触发源标记）                                                                  | 枚举：`idip_milestone` / `episode_highlight_score` / `user_starred` / `persona_result` |
| 9   | `privacy_level`  | 记忆系统            | 用户默认（来自 `user_preferences.privacy_grants`）+ 用户可改                            | 枚举：`private` / `shareable`；私有不允许进分享卡片生成                                             |
| 10  | `pinned`         | 记忆系统            | 用户输入直接                                                                       | 置顶 bool                                                                             |
| 11  | `evidence_ids[]` | 记忆系统            | 系统直接                                                                         | 指向底层 `episode` / `game_event` / `idip_milestone` 的 ID（用于回溯，分享卡片 / 复盘）               |


> **不含字段**：`game`（桌宠绑定单游戏前提）/ `pet_observation`（消费侧生成，§4.18 处理）。
...
3. 跨桌宠 IP：highlight_event 是数据层，**不受 IP 影响**；展示文案（pet_observation）才受 IP，在 消费侧生成。
...
1. 这是**用户主动设置**，**全字段 记忆系统提供**（用户输入直接存）；个别字段（content_type）可由用户反馈学习推荐，但**显式开关永远优先**。
...
#### 4.14.2 期望字段（5 子对象，**全部记忆系统提供**：用户输入直接存 + 系统反馈记录）
...
| 编号  | 字段                                         | 来源              | 产出方式                                   | 桌宠 / 画像页用途                                                                                                    |
| --- | ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 1   | `profile.gameplay_motivation[]`            | 记忆系统（用户多选确认结果） | UI 写入                                  | 模块 3"我的游戏目标"；枚举：`rank` / `chill` / `social` / `collect` / `story` / `practice_char` / `play_with_friends`；多选  |
| 2   | `profile.gameplay_motivation_candidates[]` | 消费侧派生           | LLM 从 chat + behavior_patterns 推断候选    | UI 展示候选给用户勾选；**与 #1 字段独立**（候选可能比已确认多）                                                                         |
| 3   | `profile.gameplay_style_tags[]`            | 消费侧派生 → 用户可改   | LLM 推断 + behavior + idip 规则派生；用户改后覆盖   | 模块 2"我的游戏习惯"；枚举：`outcome_sensitive_high/mid/low` / `solo_lean` / `squad_lean` / `growth_stage_early/mid/late` |
...
> **记忆系统侧字段**：assessment_id / assessment_at / data_window / user_feedback / evidence_strength（系统记录 + 用户反馈输入）。
> **消费侧派生字段**：persona_label / persona_schema_version / similarity_breakdown / is_expired（公司外部平台计算后写回 memory 持久化）。

| 编号  | 字段                       | 来源     | 产出方式                                    | 桌宠 / 画像页用途                                                                                                   |
| --- | ------------------------ | ------ | --------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| 1   | `assessment_id`          | 记忆系统   | 系统生成                                    | 主键                                                                                                           |
| 2   | `persona_label`          | 消费侧派生  | 公司既有外部平台计算 → 写回 memory 持久化              | 该游戏人格标签（如"团战指挥型"）                                                                                            |
| 3   | `persona_schema_version` | 消费侧派生  | 公司平台返回                                  | 分类体系版本（防 meta 漂移；版本升级时旧结果标过期）                                                                                |
| 4   | `similarity_breakdown[]` | 消费侧派生  | 公司平台计算                                  | `[{candidate, score (0-1), evidence_ids[]}]`；候选人格相似度 + 证据 ID 指向 game_event / idip_milestone（用于"为什么像 TA"页面展开） |
| 5   | `assessment_at`          | 记忆系统   | 系统直接                                    | 测定时间                                                                                                         |
| 6   | `data_window`            | 记忆系统   | 系统直接                                    | `{from, to, session_count}`；测定基于的数据窗口（"最近 30 天 60 局"）                                                        |
| 7   | `user_feedback`          | 记忆系统   | 用户输入直接                                  | enum：`accepted` / `rejected` / `not_like_me`；反哺下次测定权重                                                        |
| 8   | `evidence_strength`      | 记忆系统   | 系统直接（统计 evidence_ids 条数）                | 支持证据条数（**≥3 红线**）                                                                                            |
| 9   | `is_expired`             | 消费侧派生  | 规则（`persona_schema_version` 与当前版本对比）    | bool；版本升级后旧结果标过期（UI 提示"旧版本测定，是否重测"）                                                                          |
...
memory.game_persona_assessment{} (stored 结果)
                  │
                  ▼
画像页模块 10 / 11 UI 展示（消费侧生成消费侧 LLM 生成"为什么像 TA"文案）
```
...
1. **全部消费侧派生**：记忆系统提供原始计数源（game_session / episode / highlight_event / persona_assessment 表数据 + first_install_at 时间戳），消费侧用 SQL count / 时间差 / 加权公式实时算（或缓存）。
2. **PM 边界**（沿用用户清单）：纯纪念性，**不做强压力任务**；不出现"未完成任务 / 必须达成 X 才能解锁"压力 UI。
3. 关系等级的计算公式由 Engineering 决（PM 不锁数字），但 PM 守住"加权指标都来自数据层 + 不掺非数据指标"原则。

#### 4.17.2 期望字段（**全部消费侧派生**；记忆系统侧仅提供 raw 计数源）

| 字段                         | 类型       | 来源     | 派生源（从记忆系统拿什么）                                            |
| -------------------------- | -------- | ------ | ------------------------------------------------------ |
| `companion_days`           | int      | 消费侧派生  | first_install_at → now（按日历日）                           |
| `co_game_session_count`    | int      | 消费侧派生  | count(game_event.session)                              |
| `diary_count`              | int      | 消费侧派生  | count(episode where highlight_score ≥ diary_threshold) |
| `highlight_count`          | int      | 消费侧派生  | count(highlight_event WHERE deleted_at IS NULL)        |
| `persona_assessment_count` | int      | 消费侧派生  | count(game_persona_assessment)                         |
| `relationship_level`       | int      | 消费侧派生  | 上述指标加权派生（公式 Engineering 决）                             |
| `milestone_unlocked[]`     | string[] | 消费侧派生  | 派生：陪伴 7 / 30 / 100 / 365 天；首条高光；首次测定；首条日记 等等           |
...
### 4.18 消费侧派生 / 生成清单（v2.6 — 派生工作责任归消费侧）

> 此节列出**所有派生 / 生成工作**。Engineering / Design 接手时按本表实现各消费侧服务。**派生工作的责任归消费侧**，结果是否写回 memory schema 是实现优化（详见 §0.9.3），不改变归属。

#### 4.18.1 ① 派生计算（规则代码 / 计数 / 信号处理 / 加权派生）

| #   | 字段 / 视图                                                                | 触发                | 前置数据（从记忆系统拿）                              | 实现                          | 是否写回 memory |
| --- | --------------------------------------------------------------------- | ----------------- | -------------------------------------------- | -------------------------- | ----------- |
...
| 4   | `audio A0`（BPM / energy / beat_points / mute_state）                  | 音频开启时实时           | 系统音频流（本地处理，不持久化原音频）                          | 信号处理库（aubio / BeatNet / cpal） | ❌ 仅 short-term buffer ≤30s |
| 5   | `episode.highlight_score`                                             | episode 闭合时        | idip_milestone + emotion_strength + user_action | 规则加权                        | ✅ 写回 episode |
| 6   | `game_session.duration_signal`（>30min / >1h / >3h）                  | game_session 期间   | game_session.start                            | 时间差派生                       | ✅ 写回         |
...
| 10  | `relationship_stats.*`（companion_days / counts / level / milestone） | 用户查看 / 周期性        | 各表 count + first_install_at                  | SQL count + 加权 + 阈值        | ❌ 实时视图 / 缓存  |

#### 4.18.2 ② LLM 派生（AI 抽取 / 摘要 / 推断 / 分类）

| #   | 字段                                                          | 触发                  | 前置数据（从记忆系统拿）                                  | 实现                                  | 是否写回 memory       |
| --- | ----------------------------------------------------------- | ------------------- | -------------------------------------------------- | ----------------------------------- | --------------- |
| 1   | `atomic_facts[]`                                            | chat segment 闭合     | chat_text 原文 + profile context                    | AI Agent（LLM 抽取，AI Eval §3.1）        | ✅ 写回             |
| 2   | `episode.title / content`                                   | chat segment 闭合     | atomic_facts + chat_text + game_event              | AI Agent（LLM 摘要）                     | ✅ 写回             |
| 3   | `profile.summary / interests / preferences / behavior_patterns / personality_traits / key_facts` | episode 累积 / 周期性 / 用户触发 | episode + atomic_facts + behavior_patterns 累积   | AI Agent（LLM 生成 + 用户编辑后覆盖）          | ✅ 写回             |
| 4   | `profile.gameplay_motivation_candidates[]`                  | 每 7 天 / chat 累积阈值   | chat + behavior_patterns                          | AI Agent（LLM 推断候选）                    | ✅ 写回（UI 展示前置）   |
| 5   | `profile.gameplay_style_tags[]`                             | 每 N 局后 / 行为更新      | behavior + idip + game_event                       | AI + 规则混合派生                          | ✅ 写回             |
| 6   | `emotion_signal`                                            | chat 实时             | chat_text 当前 segment                              | AI 推断（AI Eval §3.4）                  | ✅ 写回             |
...
| 9   | `highlight_event` 的 `title / scene / event_summary / category / tags` | highlight 触发条件命中 | episode + idip_milestone + game_event + atomic_facts | AI Agent（LLM 合成 + 用户编辑后覆盖）          | ✅ 写回             |
| 10  | `atomic_facts.quote_eligible`                               | atomic_facts 生产时   | atomic_facts 内容                                    | 规则 PII 检测（正则 + NER） + 可选 LLM 二次过滤     | ✅ 写回             |

#### 4.18.3 ③ 外部平台派生（VLM 模型 / 公司既有平台）

| #   | 字段                                                                              | 触发              | 前置数据                                            | 实现                          | 是否写回 memory   |
| --- | ------------------------------------------------------------------------------- | --------------- | ----------------------------------------------- | --------------------------- | ----------- |
| 1   | `game_vlm.semantic_tags[]` / `user_visible_summary`                             | VLM opt-in app 前台 | active_app + 视觉帧 buffer ≤60s（本地，不持久化）          | VLM 推理服务（CNN 初筛 + 量化 7-8B VLM + 云端兜底 — AI Eval §3.6） | ✅ 写回（不存原图） |
...
#### 4.18.4 ④ 实时动态生成（不进 memory schema，按桌宠 IP / 场景每次生成）

> 这 8 项是**所有 v2.6 之前定义的"消费侧生成"**：受桌宠 IP / 用户偏好 / 当下场景影响，**stored 后失效**。

| #   | 内容                                   | 触发时机         | 前置数据（从 memory 拿）                                                                                                            | 实现责任方                | 生成约束                                                           |
| --- | ------------------------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------- | -------------------- | -------------------------------------------------------------- |
...
| 8   | **profile.summary 重写**（用户点"重新总结一下我"） | 用户点击按钮 / 周期性 | profile 全字段 + 最近 episode + highlight_event                                                                                    | LLM agent            | **例外**：写回 memory.profile.summary（记忆系统常驻 + 消费侧触发更新）             |

#### 4.18.5 PM 红线（消费侧派生 / 生成）

| #   | 红线                                  | 理由                                                                                                                              |
| --- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **记忆系统不做派生**（派生工作责任归消费侧）           | 派生 = 业务逻辑层；记忆系统是数据底座。写回 memory 是实现优化（性能 / 一致性）不改变归属                                                                                |
| 2   | **④ 类（IP 实时生成）默认不写回 memory**（例外见 #8） | 受 IP / 场景影响，写回失效                                                                                                                |
| 3   | **桌宠 IP 语气内容只走 ④ 类**               | pet_observation / 对话 / 日记正文 / 分享文案 / 卡片文案 全部按 IP 实时生成                                                                           |
| 4   | **必须基于 memory 实证，禁止 LLM 编造**       | 引用 highlight_event 时必须用 evidence_ids 回溯实证；人格"为什么像"必须基于 similarity_breakdown.evidence_ids；日记记录必须基于 atomic_facts (quote_eligible) |
| 5   | **生成失败不静默兜底**                       | LLM 推理失败 → UI 显示"内容生成中"或"暂时不可用"；不允许"假装生成成功"                                                                                     |
...
### 11.15 高光事件 highlight_event（v2.6 新增，对应 §4.13；记忆系统提供全字段）
...
7. **不含字段**：`game`（桌宠绑定单游戏前提）/ `pet_observation`（消费侧生成，§4.18 #3 消费侧每次按 IP 生成）。
8. `event_summary` 引用 atomic_facts 时必须使用 `quote_eligible = true` 子集（PII 检测）。

---

### 11.16 用户偏好 user_preferences（v2.6 新增，对应 §4.14；记忆系统提供全字段，5 子对象）
...
1. **全 记忆系统提供用户输入**；除 `content_type.feedback_signal[]` 是系统自动写（用户对内容点赞 / 点踩反馈），其余字段全部用户主动设置。
...
### 11.18 关系成长派生视图 relationship_stats（v2.6 新增，对应 §4.17；记忆系统提供派生视图）
