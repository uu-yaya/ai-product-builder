...
|---|---|---|---|
| **记忆系统提供** | 记忆系统**直接产出** | ✅ 是 | query API 直接拿，无需二次加工 |
| **消费侧生成** | memory 提供**前置数据**，消费方用 LLM / 代码**二次转换** | ❌ 否 | 拿前置数据 + 当下场景 → 动态生成 |

#### 三层数据架构

```
┌────────────────────────────────────────────────────────┐
│ Memory 采集层（记忆系统直接采集，进 schema）                │
│ active_app / chat_text / game_event / behavior_*       │
│ idip_snapshot / audio_derived / mcp_* / os_api / 等等  │
└────────────────────────────────────────────────────────┘
                        ↓
┌────────────────────────────────────────────────────────┐
│ Memory 派生层（记忆系统派生 + AI 派生结果 stored，进 schema）│
...
└────────────────────────────────────────────────────────┘
                        ↓
┌────────────────────────────────────────────────────────┐
│ 消费侧动态生成（不进 memory schema，每次按场景 / IP 生成）   │
...
| 4 | 消费侧生成生成不写回 memory（profile.summary 重写例外） | 受 IP / 场景影响，写回失效 |
...
|---|---|---|---|---|---|
| **3.1** | 聊天数据 chat | atomic_facts / episode / profile / emotion_signal / quote_eligible | 记忆系统 | "知心好友"核心；长期记忆、个性化对话、避免假记忆 | **P0** |
| **3.2** | 行为数据 - PC 进程 | active_app / idle_signal / app_switch / fullscreen | 记忆系统 | "在游戏中"vs"在工作中"判断；BOSS 战不打断 | **P0** |
| **3.3** | 行为数据 - UI & 用户操作 | window_title_redacted / 派生输入指标（A1） / 快捷键事件（A2） / 编辑动作（A3，L1.5） | 记忆系统 | 语义级"用户在做什么"；不触 keylog 红线 | **P0** + **P1** |
| **3.4** | APP 信号 - 6 通道 | MCP / OS API / 浏览器扩展 / OS Scripting Bridge / CLI / IFTTT | 记忆系统 | 接入工作 / 娱乐 app；元数据级；6 通道并存 | **P1**（MCP / OS API / 浏览器 / OSA） + **P2**（CLI / IFTTT） |
| **3.5** | 游戏数据 - idip 状态 | idip_snapshot / delta / anomaly / milestone | 记忆系统 | "游戏搭子"最高 ROI 源；首方合规 | **P0** |
| **3.6** | 游戏数据 - 实时事件 | game_session / game_event.stream / duration_signal / event_emotion_tag | 记忆系统 | 即时反应"刚死 / 刚通关"；事件级情绪锚（v2.6 新增） | **P0** |
| **3.7** | VLM 视觉理解 | semantic_tags / user_visible_summary（前台单 app 实例 opt-in） | 记忆系统 | 陪玩 / 陪看视频；三档混合（CNN + 量化 7-8B VLM + 云端兜底） | **P1** |
| **3.8** | 当前上下文 current_context | activity_topic / mood_estimate / interrupt_suitability / attention_target / confidence | 记忆系统 | 此刻该不该开口的统一决策入口 | **P0** |
| **3.9** | Profile 元字段 profile_meta | confidence / source_category / first_seen / user_corrected / decay_score / feedback_history | 记忆系统 | 区分"用户说过"vs"LLM 猜的"；避免假记忆 | **P0** |
| **3.10** | 系统音频派生信号 | A0（BPM / 能量 / 节拍） + A1（Now Playing 标识） | 记忆系统 | "随音乐舞动"愿景；不录原音频 | **P1** |
| **3.11** | Playwright 受限放行 | tool_call 一次性结果（曲名 / 收藏页 / 已登录页） | 记忆系统 | 用户主动查询的细节兜底 | **P2** |
| **3.12** | OS Scripting Bridge | osascript / PowerShell COM；仅读元数据 | 记忆系统 | 替代缺失 MCP 的工作 / 娱乐 app 通道 | **P1** |
| **3.13** | 高光事件 highlight_event | 高光记忆显式数据类（v2.6） | 记忆系统 | 日记沉淀 + 画像页"我们的高光记忆"模块 | **P1** |
| **3.14** | 用户偏好 user_preferences | 5 子对象（companion_style / emotion_response / content_type / diary_style / privacy_grants） | 记忆系统 | 画像页"陪伴方式 / 情绪偏好 / 内容偏好 / 日记风格 / 隐私授权" | **P1** |
| **3.15** | 用户画像 profile 扩展 | gameplay_motivation[] / gameplay_motivation_candidates[] / gameplay_style_tags[] | 记忆系统 | 画像页"游戏目标 / 游戏习惯"枚举标签 | **P1** |
| **3.16** | 游戏人格测定 game_persona_assessment | persona_label / similarity_breakdown / evidence_strength / user_feedback | 记忆系统 | 画像页"游戏人格测定"+ 分享卡片；接入公司既有平台 | **P1 候选**（待 4 条前置） |
| **3.17** | 关系成长视图 relationship_stats | companion_days / counts / relationship_level / milestone_unlocked（派生） | 记忆系统 | 画像页"关系成长系统"纪念性展示 | **P1** |
| **3.18** | 消费侧动态生成（**不进 memory schema**） | 日记正文 / pet_observation / 对话回复 / 分享卡片文案 / 等 8 项 | 消费侧 | 受桌宠 IP / 场景影响每次生成 | 各消费方实现 |
...
> 每个模块包含：**说明**（用途 + 来源驱动）/ **字段表**（编号 / 字段 / 来源 / 类型 / 派生方式 / 优先级 / 用途）/ **边界**（如有隐私 / 红线）/ **Schema 示例**（简化 JSON；完整 mock 索引见 § 4.4）。
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `atomic_facts[]` | 记忆系统 | 数组（structured） | AI 派生层（LLM 抽取） | P0 | 短期对话引用："你刚说过 …" |
| 2 | `episode.title` + `episode.content` + `episode.time_range` + `episode.participants` | 记忆系统 | 对象 | AI 派生层（LLM 摘要） | P0 | 跨日 / 跨 session 召回 |
| 3 | `profile.summary` | 记忆系统常驻 + 消费侧触发更新 | string | LLM 生成 + 用户点"重新总结"触发重写 | P0 | 风格定调 |
| 4 | `profile.interests[]` / `profile.preferences[]` | 记忆系统 | 数组 | LLM 抽取 + ✅ 用户可改 | P0 | 主动话题选取 |
| 5 | `profile.behavior_patterns[]` | 记忆系统 | 数组 | LLM 抽取 | P0 | 时机判断 |
| 6 | `profile.key_facts[]` | 记忆系统 | 数组 | LLM 候选 + ✅ 用户确认才生效 | P0 | 重要事实优先注入对话上下文 |
| 7 | `profile.personality_traits[]` | 记忆系统 | 数组 | LLM 推断 + ✅ 用户"不像我"标记（不可直接编辑） | P0 | 个性描述 |
| 8 | `emotion_signal` | 记忆系统 | enum (紧张/兴奋/沮丧/平静) | AI 派生（AI Eval § 3.4） | P0 | 决定语气 |
| 9 | `atomic_facts.quote_eligible`（v2.6 新增） | 记忆系统 | bool | 派生层 PII 检测（正则 + NER） | P0 | 日记引用原话的隐私安全开关 |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `active_app.name` / `bundle_id` / `process_id` | 记忆系统 | 对象 | OS 进程 API 采集 | P0 | "在游戏中"vs"在工作中" |
| 2 | `active_app.is_fullscreen` | 记忆系统 | bool | OS 窗口 API | P0 | 打扰决策："全屏 = 闭嘴" |
| 3 | `idle_signal`（>5min / >10min / >30min 分级） | 记忆系统 | enum | 派生（鼠标 / 键盘 idle 时间） | P0 | "用户离开"判断 |
| 4 | `app_switch_burst` | 记忆系统 | bool / int | 派生（短期切换频率） | P0 | "频繁切换 = 焦虑" |
| 5 | `recent_apps_top3` | 记忆系统 | string[] | 派生（最近 1h 时长 top3） | P0 | "看你今天一直在写代码" |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `window_title_redacted` | 记忆系统 | string | OS 窗口 API + 脱敏 | P0 | "在 IDE 编辑代码文件" |
| 2 | `ui_text_snapshot`（白名单 app + opt-in） | 记忆系统 | string | OS Accessibility API + buffer | P1 | "屏幕上有错误弹窗" |
| 3 | `focused_element_role` | 记忆系统 | enum | OS 控件 API | P1 | "正在输入"不打扰 |
| 4 | A1 派生指标：`input_intensity_level` / `typing_rhythm_signal` / `mouse_activity_burst` / `mouse_region_heatmap_top3` / `scroll_intensity_signal` 等 | 记忆系统 | 各类型 | 派生（桶化） | P0 + P1 | 专注 vs 休息判断 |
| 5 | A2 语义事件：`semantic_events_v2_5[]`（save / copy / undo / fullscreen / lock / new_window / new_tab / app_install / window_arrange） | 记忆系统 | 数组 | OS Event API（NSApp / Win EVENT_SYSTEM_*） | P1 | "刚保存 / 刚切 app" |
| 6 | A3 编辑派生：`text_edit_action_burst` / `undo_redo_rate_per_min` / `ime_state` / `editing_session_duration_min` / `text_edit_burst_frequency` | 记忆系统 | 各类型 | 派生 | P1 | "在编辑长文档"等 |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `idip_snapshot` | 记忆系统 | 对象 | 游戏 SDK 推送 | P0 | 角色等级 / 段位 / 通关进度等基础状态 |
| 2 | `idip_delta` | 记忆系统 | 对象 | 派生（与上一时点对比） | P0 | "刚通关" / "段位掉了" / "装备升级" |
| 3 | `idip_anomaly` | 记忆系统 | 对象 | 派生（异常模式识别） | P0 | "3 次同一处失败" / "卡关 5min" |
| 4 | `idip_milestone[]` | 记忆系统 | 数组 | 派生（成就 / 突破点） | P0 | 桌宠主动祝贺的触发点 |
| 5 | `idip_field_metadata` | 记忆系统 | 对象 | 配置 | P0 | 字段语义说明（数字含义） |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `game_session.start` / `game_session.end` | 记忆系统 | event | 游戏 SDK | P0 | 知道一局开始结束 |
| 2 | `game_session.in_game_time` | 记忆系统 | timestamp | 游戏 SDK 实时 | P0 | 游戏内时间 |
| 3 | `game_event.stream[]`（open / death / revive / settlement / ...） | 记忆系统 | 事件流 | 游戏 SDK pub/sub | P0 | 桌宠按事件反应 |
| 4 | `game_session.duration_signal`（>30min / >1h / >3h） | 记忆系统 | enum | 派生 | P0 | 健康提示触发 |
| 5 | **`event_emotion_tag`**（v2.6 新增） | 记忆系统 | enum (joy / frustrate / satisfy / sadness / neutral) | 派生层 AI（复用 § 3.1 emotion_signal 模型扩到事件级） | P0 | 日记 / 高光 / 复盘的事件级情绪锚点 |
| 6 | **`episode.highlight_score`**（v2.6 新增；派生到 § 3.1 episode） | 记忆系统 | float (0-1) | 派生层规则代码（idip_milestone × emotion_strength × user_action 加权） | P0 | 日记筛选阈值 + 高光候选输入 |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `vlm_enabled_for_this_app_instance` | 记忆系统 | bool | 用户设置 | P1 | 每个 app 实例独立开关 |
| 2 | `app_category` | 记忆系统 | enum (game / video) | 用户配置 | P1 | 区分游戏类 vs 视频类规则 |
| 3 | `semantic_tags[]` | 记忆系统 | string[] (≤5) | VLM 派生 | P1 | 场景标签（boss_fight / low_hp / scene_funny / 等） |
| 4 | `user_visible_summary` | 记忆系统 | string (≤50 字) | VLM 派生 | P1 | "刚才那段是搞笑剧情" |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `activity_topic` | 记忆系统 | string | 派生（滑窗摘要） | P0 | 桌宠对话的话题锚 |
| 2 | `mood_estimate` | 记忆系统 | enum (紧张 / 平静 / 兴奋 / 沮丧) | 派生（滑窗 AI） | P0 | 决定语气 |
| 3 | `interrupt_suitability` | 记忆系统 | enum (high / medium / low) | 派生（综合判断） | P0 | 打扰决策唯一输入 |
| 4 | `attention_target` | 记忆系统 | enum (游戏 / IDE / 视频 / ...) | 派生（实时） | P0 | 桌宠决定看哪个屏幕方向 |
| 5 | `confidence` | 记忆系统 | float | 元字段 | P0 | 低置信时桌宠保守 |
| 6 | `trigger` | 记忆系统 | enum (activity_topic_change / mood_change / interrupt_change / heartbeat) | 系统 | P0 | 推送原因 |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `confidence` | 记忆系统 | float (0-1) | 派生（规则 + LLM 混合，min 保守） | P0 | 桌宠引用前判断"是否说出来" |
| 2 | `source_category` | 记忆系统 | enum (chat / user_confirmed / game_event / behavior / mcp / vlm / llm_inferred) | 系统 | P0 | Memory Center 来源解释 |
| 3 | `first_seen_at` / `last_confirmed_at` | 记忆系统 | timestamp | 系统 | P0 | 时效衰减 |
| 4 | `user_corrected` | 记忆系统 | bool | 用户操作 | P0 | 纠错过的不再自动写回 |
| 5 | `decay_score` | 记忆系统 | float | 派生（时间衰减） | P0 | 桌宠不用自己做衰减逻辑 |
| 6 | **`feedback_history[]`**（v2.6 新增） | 记忆系统 | 数组 [{feedback_type, at, weight_impact}] | 系统自动写 | P1 | 反馈反哺闭环（画像模块 15）|
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | A0 `bpm` | 记忆系统 | float | 本地音频处理（aubio / BeatNet） | P1 | 节拍 sync |
| 2 | A0 `energy_curve` | 记忆系统 | float[] | 本地音频处理 | P1 | 强度变化 |
| 3 | A0 `beat_points[]` | 记忆系统 | timestamp[] | 本地音频处理 | P1 | 节拍点 |
| 4 | A0 `mute_state` | 记忆系统 | bool | OS API | P1 | 静音感知 |
| 5 | A1 `now_playing.title` / `artist` / `duration` / `position` | 记忆系统 | 对象 | OS MediaRemote (macOS) / SMTC (Win) | P1 | "刚才那首歌叫什么" |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `tool_call_id` | 记忆系统 | string | 系统 | P2 | 一次性查询 ID |
| 2 | `result_snippet` | 记忆系统 | string | Playwright + 用户主动触发 | P2 | "刚才那首歌叫什么"答案 |
| 3 | `ttl_until` | 记忆系统 | timestamp | 系统（buffer ≤5min） | P2 | 短期 buffer |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `bridge_kind` | 记忆系统 | enum (osascript_applescript / powershell_com) | 系统 | P1 | macOS / Win 平台 |
| 2 | `user_authorized_apps[]` | 记忆系统 | string[] | 用户授权 | P1 | 桌宠允许 OSA 接入的 app |
| 3 | `samples[]` | 记忆系统 | 数组 [{app, field, value, ttl_until}] | osascript / COM 调用 | P1 | 元数据样本 |
| 4 | `ui_indicator_shown_per_app[]` | 记忆系统 | string[] | 合规检查 | P1 | UI 启用指示 |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `highlight_id` | 记忆系统 | string | 系统 | P1 | 主键 |
| 2 | `title` | 记忆系统 | string | LLM 候选 + ✅ 用户可编辑 | P1 | 标题（≤20 字） |
| 3 | `time` | 记忆系统 | timestamp | 系统 | P1 | 事件时间 |
| 4 | `scene` | 记忆系统 | string | LLM 派生（active_app + window_title + game_vlm.tags 合成） | P1 | 场景描述 |
| 5 | `event_summary` | 记忆系统 | string | LLM 生成 + ✅ 用户可编辑 | P1 | 摘要（≤80 字；引用 atomic_facts 必须 quote_eligible=true） |
| 6 | `category` | 记忆系统 | enum (achievement / growth / emotion / social / collection / relationship) | LLM 推荐 + ✅ 用户可改 | P1 | 高光分类 |
| 7 | `tags[]` | 记忆系统 | string[] (≤5) | LLM 候选 + ✅ 用户可改 | P1 | 自由标签 |
| 8 | `source` | 记忆系统 | enum (idip_milestone / episode_highlight_score / user_starred / persona_result) | 系统 | P1 | 触发来源 |
| 9 | `privacy_level` | 记忆系统 | enum (private / shareable) | 用户默认（来自 user_preferences.privacy_grants）+ ✅ 可改 | P1 | 分享性 |
| 10 | `pinned` | 记忆系统 | bool | 用户输入 | P1 | 置顶 |
| 11 | `evidence_ids[]` | 记忆系统 | string[] | 系统（指向 episode / game_event / idip_milestone / atomic_facts ID） | P1 | 反查实证（分享卡片 / 复盘） |
...
1. **不含字段**：`game`（桌宠绑定单游戏前提）/ `pet_observation`（消费侧生成，§ 3.18 处理，受桌宠 IP 影响每次按 IP 生成）。
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `profile.gameplay_motivation[]` | 记忆系统 | enum 多选 (rank / chill / social / collect / story / practice_char / play_with_friends) | ✅ 用户多选确认 | P1 | 模块 3"我的游戏目标" |
| 2 | `profile.gameplay_motivation_candidates[]` | 记忆系统 | enum 多选 | LLM 推断候选（与 #1 字段独立） | P1 | UI 展示候选给用户勾选 |
| 3 | `profile.gameplay_style_tags[]` | 记忆系统 | enum 多选 (outcome_sensitive_high/mid/low / solo_lean / squad_lean / growth_stage_early/mid/late) | LLM + behavior 派生 + ✅ 用户可改 | P1 | 模块 2"我的游戏习惯" |
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `assessment_id` | 记忆系统 | string | 系统 | P1 候选 | 主键 |
| 2 | `persona_label` | 记忆系统 | string | 公司既有平台 | P1 候选 | 人格标签（如"团战指挥型"） |
| 3 | `persona_schema_version` | 记忆系统 | string | 公司平台 | P1 候选 | 分类体系版本（meta 漂移检测） |
| 4 | `similarity_breakdown[]` | 记忆系统 | 数组 [{candidate, score, evidence_ids[]}] | 公司平台 | P1 候选 | 候选人格相似度 + 证据 IDs |
| 5 | `assessment_at` | 记忆系统 | timestamp | 系统 | P1 候选 | 测定时间 |
| 6 | `data_window` | 记忆系统 | {from, to, session_count} | 系统 | P1 候选 | 测定数据窗口 |
| 7 | `user_feedback` | 记忆系统 | enum (accepted / rejected / not_like_me / null) | ✅ 用户 | P1 候选 | 反哺下次测定权重 |
| 8 | `evidence_strength` | 记忆系统 | int | 系统 | P1 候选 | 支持证据条数（**≥3 红线**） |
| 9 | `is_expired` | 记忆系统 | bool | 派生（基于 persona_schema_version） | P1 候选 | 旧版本测定过期标记 |
...
memory.game_persona_assessment{} (stored 结果)
                  │
                  ▼
画像页模块 10 / 11 UI 展示（消费侧生成消费侧 LLM 生成"为什么像 TA"文案，见 § 3.18 #6）
```
...
| # | 字段 | 来源 | 类型 | 派生方式 | 优先级 | 用途 |
|---|---|---|---|---|---|---|
| 1 | `companion_days` | 记忆系统 | int | `first_install_at` → `now`（按日历日） | P1 | 陪伴天数 |
| 2 | `co_game_session_count` | 记忆系统 | int | `count(game_event.session)` | P1 | 共同对局数 |
| 3 | `diary_count` | 记忆系统 | int | `count(episode WHERE highlight_score ≥ diary_threshold)` | P1 | 日记数 |
| 4 | `highlight_count` | 记忆系统 | int | `count(highlight_event WHERE deleted_at IS NULL)` | P1 | 高光数 |
| 5 | `persona_assessment_count` | 记忆系统 | int | `count(game_persona_assessment)` | P1 | 测定次数 |
| 6 | `relationship_level` | 记忆系统 | int | 上述指标加权派生（公式 Engineering 决） | P1 | 关系等级 |
| 7 | `milestone_unlocked[]` | 记忆系统 | string[] | 派生（7d / 30d / 100d / 365d / 首次高光 / 首次测定 等） | P1 | 已解锁里程碑 |
| 8 | `milestone_pending[]` | 记忆系统 | string[] | 派生（未达成清单） | P1 | 待解锁里程碑（纪念性） |
...
| 8 | **profile.summary 重写**（用户点"重新总结一下我"） | 用户点击按钮 / 周期性 | profile 全字段 + 最近 episode + highlight_event | LLM agent | **例外**：写回 memory.profile.summary（记忆系统常驻 + 消费侧触发更新混合） |

#### PM 红线（消费侧 消费侧生成生成）

| # | 红线 | 理由 |
|---|---|---|
| 1 | **消费侧生成生成默认不写回 memory**（例外见 #8） | 受 IP / 场景影响，写回失效 |
| 2 | **桌宠 IP 语气内容只走 消费侧生成** | pet_observation / 对话 / 日记正文 / 分享文案 / 卡片文案 全部按 IP 实时生成 |
...
| 4 | **消费侧生成生成失败不静默兜底** | LLM 推理失败 → UI 显示"内容生成中"或"暂时不可用"；不允许"假装生成成功" |
...
- [ ] "记忆系统提供 vs 消费侧生成" 划分是否合理？是否有归到"记忆系统提供"的字段实际应该是"消费侧生成"（受 IP / 场景影响）？反之亦然？
...
- [ ] § 3.18 消费侧生成生成 4 条红线是否给消费侧实现方足够约束？
